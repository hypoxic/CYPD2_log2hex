...
def main():
    if len(sys.argv) < 2:
        print("Usage: %s  <cfg file>")
        sys.exit()
        
    cfg_file = sys.argv[1]
    
    if not os.path.isfile(cfg_file):
        print("cfg file path %s does not exist. Exiting..." %  (bin_file))
        sys.exit()
    
    with open(cfg_file) as cfgfp:
        cfg = cfgfp.read()
        cfg = cfg.replace('\n', ' ').replace('\r', '')
        clist = cfg.split(",") 
    ...
    ocsum = cld[CFG_TABLE_CHECKSUM]

    # clear out the original
    cld[CFG_TABLE_CHECKSUM] = 0
    
    csum = 0
    
    for i in range(CFG_TABLE_START, CFG_TABLE_END):
        csum += cld[i]
        
    csum = twos_comp(csum)
    #replacement is here!
    cld[CFG_TABLE_CHECKSUM] = csum
    
    s = []    
    for i,c in enumerate(cld):
        if i and not (i % 0x10):
            s.append("\n")
        s.append(str("0x%02x," % c))
        

    ss = ''.join(s)    
    ss = ss.strip()    
    ss = ss[:len(ss)-1]   
    print("new csum %x" % csum)        
    print("old csum %x" % ocsum)
        
    with open("cfgout.c", mode='w') as ofp:
        ofp.write(ss)          
